...
# Trigger type → HA bus event name
_TRIGGER_EVENT: dict[str, str] = {
    "arrived":          EVENT_CITY_ARRIVED,
    "departed":         EVENT_CITY_DEPARTED,
...
def _person_entity_id(hass: HomeAssistant, device_id: str) -> str | None:
    """Return the person entity_id stored in the device identifiers, or None."""
    device = dr.async_get(hass).async_get(device_id)
    if device is None:
        return None
    for id_domain, identifier in device.identifiers:
        if id_domain == DOMAIN:
            return identifier  # stored as the person entity_id
    return None
...
async def async_get_triggers(
    ...
    """Return the triggers available for a Whereabouts device."""
